;; Mock USDC Token for StackPredict Protocol (Devnet)
;; Implements SIP-010 fungible token standard with additional functions for testing
...
(define-constant FAUCET-AMOUNT u10000000000) ;; 10,000 USDC (with 6 decimals)
...
(define-public (faucet (amount uint))
  (let
    ...
    (try! (ft-mint? mock-usdc amount caller))
...
(define-public (burn (amount uint))
  (let
    (
      (caller tx-sender)
      (balance (ft-get-balance mock-usdc caller))
    )
    (asserts! (> amount u0) ERR-ZERO-AMOUNT)
    (asserts! (>= balance amount) ERR-INSUFFICIENT-BALANCE)
    (try! (ft-burn? mock-usdc amount caller))
